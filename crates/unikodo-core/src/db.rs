...
/// Vendored, normalized tables for the file-sourced schemes, generated by
/// `scripts/update-*-symbols.py` (see `data/README.md` for sources and licenses).
const LATEX_TSV: &str = include_str!("../data/latex-symbols.tsv");
const LEAN_TSV: &str = include_str!("../data/lean-symbols.tsv");
const ROCQ_TSV: &str = include_str!("../data/rocq-symbols.tsv");
...
/// the `latex`, `lean`, and `rocq` tables. Each [`Symbol`] is tagged with its
/// [`scheme`](crate::scheme).
pub fn symbols() -> &'static [Symbol] {
    ...
    SYMBOLS
        .get_or_init(|| {
            ...
            parse_tsv(scheme::LEAN, LEAN_TSV, &mut all);
            parse_tsv(scheme::ROCQ, ROCQ_TSV, &mut all);
            all
        })
        .as_slice()
}
...
mod tests {
    ...
    #[test]
    fn lean_scheme_present() {
        assert!(count(scheme::LEAN) > 1000);
        assert_eq!(find(scheme::LEAN, "nat").value, "ℕ");
        assert_eq!(find(scheme::LEAN, "to").value, "→");
        assert_eq!(find(scheme::LEAN, "alpha").value, "α");
        // Snippet abbreviations (with a `$` placeholder) are excluded.
        assert!(symbols()
            .iter()
            .filter(|s| s.scheme == scheme::LEAN)
            .all(|s| !s.value.contains('$')));
    }

    #[test]
    fn rocq_scheme_present() {
        assert!(count(scheme::ROCQ) > 1000);
        assert_eq!(find(scheme::ROCQ, "forall").value, "∀");
        assert_eq!(find(scheme::ROCQ, "alpha").value, "α");
        assert_eq!(find(scheme::ROCQ, "to").value, "→");
    }
}
